...
?>
<div class="box">
    <h3 style=" text-align:center; color:#000; margin-top:20px; margin-bottom:20px; ">发布</h3>
    <hr align="center" /><br/>
    <?php
    //phpstorm中不显示warning
    error_reporting(0);
    ...
    if ($_GET['action']=='dj'){
        if ($status!=3) {
            echo "<script type='text/javascript'>alert('该信息未审核或已发布');history.back();</script>";
            exit;
        }
    }
    if ($_GET['action']=='add'){
        ...
        elseif (empty($_POST['title']) || empty($_POST['content'])|| empty($_FILES['file']['name'])){
            ...
            $sql = "update help set newtitle = '{$_POST['title']}',newcontent= '{$_POST['content']}',newpic= '{$_newfile}',newstarttime = '{$time2}' ,status = -1 where id = '$id'";
            $query = mysqli_query($_conn,$sql);
            echo "<script type='text/javascript'>alert('发布成功');window.location.href='help.php';</script>";
        }
    }
    ?>
