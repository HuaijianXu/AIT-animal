...
<div>
    <table class="table table-hover table-striped table-bordered table-sm" id="resultshow">
        ...
        $hid = $_COOKIE['uid'];
        if ($_GET['action']=='dj'){
            $query1 = mysqli_query($_conn,"select * from help where kind=0 and uid={$hid} and (status=-3 or status=-2 or status=-1 or status=3) order by status desc");
        }
        elseif ($_GET['action']=='ly'){
            $query1 = mysqli_query($_conn,"select * from help where kind=0 and rid={$hid} and (status=0 or status=1 or status=2) order by status desc ");
        }
        elseif ($_GET['action']=='jy'){
            $query1 = mysqli_query($_conn,"select * from help where kind=1 and uid={$hid} order by status desc");
        }
        if ($_GET['action']=='dj'){
            echo '<tr>
            <th>登记用户名</th>
