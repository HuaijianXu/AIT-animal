...
<html>
    ...
    <body background="../images/bj1.jpg">
        ...
        ?>
        <div class="box">
            <h3 style=" text-align:center; color:#000; margin-top:20px; margin-bottom:20px; ">修改货物</h3>
            <?php
            error_reporting(0);
                if ($_GET['action']=='edit'){
                    if ($_FILES['file']['name']=="") {
                        $query= mysqli_query($_conn,"update goods set title = '{$_POST['title']}',price = '{$_POST['price']}',content= '{$_POST['content']}' ,status= '{$_POST['status']}' where id = ".$_GET['id']) or die(mysqli_error());
                        echo "<script type='text/javascript'>alert('修改成功');window.location.href='admin_goods.php';</script>";
                    }
                    else{
                        //设置时区为中国
                        ...
                        $query = mysqli_query($_conn, "update goods set title = '{$_POST['title']}',price = '{$_POST['price']}',pic='{$_newfile}',content= '{$_POST['content']}',status= '{$_POST['status']}' where id = " . $_GET['id']) or die(mysqli_error());
                        echo "<script type='text/javascript'>alert('修改成功');window.location.href='admin_goods.php';</script>";
                    }
                }
            ...
            <form action="?action=edit&id=<?php echo $_GET['id'];?>" method="post" enctype="multipart/form-data">
                <table width="500" border="0"  align="center">
                    ...
                    <tr>
                        ...
                        <td>
                        <input type="radio" value="待出售" name="status">待出售
                        <input type="radio" value="买家已购买" name="status">买家已购买
                        </td>
                    </tr>
...
<script>
    window.onload = function () {
        ...
        for (var i=0;i<status.length;i++){
            if (status[i].value == "<?php echo $_rows['status'];?>"){
