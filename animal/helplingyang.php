...
?>
<div class="box">
    <h3 style=" text-align:center; color:#000; margin-top:20px; margin-bottom:20px; ">提交申请</h3>
    <hr align="center" /><br/>
    <?php
    //phpstorm中不显示warning
    error_reporting(0);
    ...
    $rid = $_COOKIE['uid'];
    $id = $_GET['id'];
    $query2 = mysqli_query($_conn,"select * from help where id = {$id}");
    $row = mysqli_fetch_array($query2);
    if ($_GET['action']=='add'){
        $id = $_GET['id'];
        $sql = "update help set status =0 ,rid={$rid},rusername='{$user}' where id = {$id}";
        $query2 = mysqli_query($_conn,$sql);
        echo "<script type='text/javascript'>alert('提交申请成功');window.location.href='help.php';</script>";
    }
    ?>
    <form action="?action=add&id=<?php echo $id?>" method="post" enctype="multipart/form-data">
        <table border="1" align="center">
            <tr>
                <td rowspan="5"><img src="./help/<?php echo $row['newpic']?>" width=200px title="imag-name" /></td>
            </tr>
            <tr>
                <td>标题：<?php echo $row['newtitle']?></td>
            </tr>
            <tr>
                <td>登记人：<?php echo $row['username']?></td>
            </tr>
            <tr>
                <td>领养人：<?php echo $user?></td>
            </tr>
            <tr>
                <td>发布时间：<?php echo $row['newstarttime']?></td>
            </tr>
            <tr>
                <td colspan="2" align="center"><textarea style="width: 100%;height: 100%"><?php echo $row['newcontent']?></textarea></td>
            </tr>
            <tr>
