...
$city = $_GET['city'];
include 'connect.php';
$sql = "select place, count(place)as num from product where city = '{$city}' group by place ";
$query = mysqli_query($_conn,$sql);
$arr = array();
while ($rows = mysqli_fetch_array($query)){
    array_push($arr,array('name'=>$rows['place'],'value'=>$rows['num']));
}
...
<script>
    ...
    $.get("mapjson/city/"+ city +".json", function (geoJson) {
        myChart.hideLoading();
...
<?php
$sql = "select * from product where city = '{$city}' order by id desc";
