...
?>
<div class="box">
    <h3 style=" text-align:center; color:#000; margin-top:20px; margin-bottom:20px; ">寄养</h3>
    <hr align="center" /><br/>
    <?php
    //phpstorm中不显示warning
    error_reporting(0);
    ...
    if ($_GET['action']=='add'){
        ...
        elseif (empty($_POST['title']) || empty($_POST['content'])|| empty($_FILES['file']['name'])){
            ...
            move_uploaded_file($_FILES["file"]["tmp_name"], "./help/" . $_newfile);
            $sql = "insert into help (hid,title,pic,content,username,starttime,uid,kind,status) values ({$hid},'{$_POST['title']}','{$_newfile}','{$_POST['content']}','{$_COOKIE['yhm']}','$time2','{$uid}',1,0)";
            $query = mysqli_query($_conn,$sql);
            echo "<script type='text/javascript'>alert('提交成功');window.location.href='help.php';</script>";
        }
    }
    ?>
    <form action="?action=add&hid=<?php echo $hid?>" method="post" enctype="multipart/form-data" >
